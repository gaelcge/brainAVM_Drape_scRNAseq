...
setwd("/home/jhowa105/projects/def-dubraca/jhowa105/singlecell/AVM/PNVPCT/analysis")

# # emptyDrops - Remove empty droplets from data #
# data_dir <- "/home/jhowa105/projects/def-dubraca/jhowa105/singlecell/AVM/PNVPCT/cellranger_outs/raw_feature_bc_matrix"
...
# np <- import("numpy")
# is_doublet <- np$load("./solo_28.06.22_15_10_25/is_doublet.npy")
# preds <- np$load("./solo_28.06.22_15_10_25/preds.npy")
sce_mat <- as.matrix(counts(sce))
colnames(sce_mat) <- sce@colData@listData$Barcode
rownames(sce_mat) <- sce@rowRanges@elementMetadata@listData$Symbol
...
seur <- subset(seur, subset = nCount_RNA < 50000 & nFeature_RNA > 200 & nFeature_RNA < 7000 & percent.mt < 10)
...
brain$Phase <- factor(brain$Phase, levels = rev(c("G1", "S", "G2M")))
brain$cluster_condition <- paste(brain$seurat_clusters, brain$condition)
brain$cluster_condition <- factor(brain$cluster_condition, levels = unlist(lapply(sort(unique(as.numeric(as.character(brain$seurat_clusters)))), function(x) paste(x, c('Control', 'Alk5iEKO')))))
png("phase_barplot.png", units = "in", height = 5, width = 20, res = 150)
ggplot(brain@meta.data) + 
...
brain$condition <- "CT"
brain <- subset(brain, subset = seurat_clusters == 5, invert = TRUE)
brain$cell_type <- "Venous Capillary ECs"
brain$cell_type[which(brain$seurat_clusters == 1)] <- "Parenchymal Stalk Cells"
brain$cell_type[which(brain$seurat_clusters == 2)] <- "Gm26917 Pos Capillary ECs"
brain$cell_type[which(brain$seurat_clusters == 3)] <- "Venous ECs"
brain$cell_type[which(brain$seurat_clusters == 4)] <- "Tip Cells"
brain$cell_type[which(brain$seurat_clusters == 6)] <- "Proliferative ECs"
brain$cell_type[which(brain$seurat_clusters == 7)] <- "Arterial ECs"
brain$cell_type[which(brain$seurat_clusters == 8)] <- "Choroids"
