...
setwd("/home/jhowa105/projects/def-dubraca/jhowa105/singlecell/AVM/BrainMut/analysis")

# # emptyDrops - Remove empty droplets from data #
# data_dir <- "/home/jhowa105/projects/def-dubraca/jhowa105/singlecell/AVM/BrainMut/cellranger_outs/raw_feature_bc_matrix"
...
# np <- import("numpy")
# is_doublet <- np$load("./solo_28.06.22_15_09_43/is_doublet.npy")
# preds <- np$load("./solo_28.06.22_15_09_43/preds.npy")
sce_mat <- as.matrix(counts(sce))
colnames(sce_mat) <- sce@colData@listData$Barcode
rownames(sce_mat) <- sce@rowRanges@elementMetadata@listData$Symbol
...
seur <- subset(seur, subset = nCount_RNA < 50000 & nFeature_RNA > 200 & nFeature_RNA < 7500 & percent.mt < 10)
...
seur$condition <- "MUT"
seur <- subset(seur, subset = seurat_clusters == 8, invert = TRUE)
seur$cell_type <- "Tip Cell Like ECs"
seur$cell_type[which(seur$seurat_clusters == 1)] <- "Arterial Capillary ECs"
seur$cell_type[which(seur$seurat_clusters == 2)] <- "Venous Capillaries"
seur$cell_type[which(seur$seurat_clusters == 3)] <- "Venous ECs"
seur$cell_type[which(seur$seurat_clusters == 4)] <- "Tip Cells"
seur$cell_type[which(seur$seurat_clusters == 5)] <- "Proliferative ECs"
seur$cell_type[which(seur$seurat_clusters == 6)] <- "Gm26917 Pos Capillary ECs"
seur$cell_type[which(seur$seurat_clusters == 7)] <- "Capillary ECs"
seur$cell_type[which(seur$seurat_clusters == 9)] <- "Arterial ECs"
seur$cell_type[which(seur$seurat_clusters == 10)] <- "Choroids"
