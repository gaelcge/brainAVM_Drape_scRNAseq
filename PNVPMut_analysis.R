...
setwd("/home/jhowa105/projects/def-dubraca/jhowa105/singlecell/AVM/PNVPMut/analysis")

# # emptyDrops - Remove empty droplets from data #
# data_dir <- "/home/jhowa105/projects/def-dubraca/jhowa105/singlecell/AVM/PNVPMut/cellranger_outs/raw_feature_bc_matrix"
...
np <- import("numpy")
is_doublet <- np$load("./solo_28.06.22_15_19_50/is_doublet.npy")
preds <- np$load("./solo_28.06.22_15_19_50/preds.npy")
sce_mat <- as.matrix(counts(sce))
colnames(sce_mat) <- sce@colData@listData$Barcode
rownames(sce_mat) <- sce@rowRanges@elementMetadata@listData$Symbol
...
seur <- subset(seur, subset = nCount_RNA < 50000 & nFeature_RNA > 200 & nFeature_RNA < 6500 & percent.mt < 10)
...
brain$condition <- "MUT"
brain <- subset(brain, subset = seurat_clusters == 7, invert = TRUE)
brain$cell_type <- "Venous Capillary ECs"
brain$cell_type[which(brain$seurat_clusters == 1)] <- "Parenchymal Capillary ECs"
brain$cell_type[which(brain$seurat_clusters == 2)] <- "Venous ECs"
brain$cell_type[which(brain$seurat_clusters == 3)] <- "Tip Cells"
brain$cell_type[which(brain$seurat_clusters == 4)] <- "Vein Like ECs"
brain$cell_type[which(brain$seurat_clusters == 5)] <- "Proliferative ECs"
brain$cell_type[which(brain$seurat_clusters == 6)] <- "Arterial ECs"
brain$cell_type[which(brain$seurat_clusters == 8)] <- "Gm26917 Pos Capillary ECs"
brain$cell_type[which(brain$seurat_clusters == 9)] <- "Parenchymal Stalk Cells"
brain$cell_type[which(brain$seurat_clusters == 11)] <- "Choroids"
brain$cell_type[which(brain$seurat_clusters == 12)] <- "Proliferative ECs"
